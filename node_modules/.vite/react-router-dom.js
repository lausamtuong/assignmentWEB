...
function createMemoryHistory(b) {
  function h(d, g) {
    g === void 0 && (g = null);
    return C2(_extends2({ pathname: t.pathname, search: "", hash: "" }, typeof d === "string" ? J2(d) : d, { state: g, key: H2() }));
  }
  function e(d, g, c) {
  ...
  function z(d, g) {
    var c = r2.Push, a = h(d, g);
    true ? D2(t.pathname.charAt(0) === "/", "Relative pathnames are not supported in memory history.push(" + JSON.stringify(d) + ")") : void 0;
    e(c, a, function() {
      z(d, g);
    }) && (m += 1, p.splice(m, p.length, a), x(c, a));
  }
  function A(d, g) {
    var c = r2.Replace, a = h(d, g);
    true ? D2(t.pathname.charAt(0) === "/", "Relative pathnames are not supported in memory history.replace(" + JSON.stringify(d) + ")") : void 0;
    e(c, a, function() {
      A(d, g);
    }) && (p[m] = a, x(c, a));
  }
  function y(d) {
    var g = Math.min(Math.max(m + d, 0), p.length - 1), c = r2.Pop, a = p[g];
    e(c, a, function() {
      y(d);
    }) && (m = g, x(c, a));
  }
  ...
  var p = (b === void 0 ? ["/"] : b).map(function(d) {
    var g = C2(_extends2({ pathname: "/", search: "", hash: "", state: null, key: H2() }, typeof d === "string" ? J2(d) : d));
    true ? D2(g.pathname.charAt(0) === "/", "Relative pathnames are not supported in createMemoryHistory({ initialEntries }) (invalid entry: " + JSON.stringify(d) + ")") : void 0;
    return g;
  }), m = Math.min(Math.max(w == null ? p.length - 1 : w, 0), p.length - 1), u = r2.Pop, t = p[m], v = F2(), q = F2();
  return { get index() {
    return m;
  }, get action() {
    return u;
...
var Link = (0, import_react2.forwardRef)(function LinkWithRef(_ref4, ref) {
  ...
  return (0, import_react2.createElement)("a", _extends3({}, rest, {
    href,
    onClick: handleClick,
    ref,
    target
